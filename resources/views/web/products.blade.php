        ...
        <div class="container">
            <div class="row justify-content-center mb-2 pb-2">
                <div class="col-md-8 text-center heading-section ftco-animate">
                    <h2 style="color:rgba(223,223,223,.3);margin-bottom:-50px;">{{ __('links.advanced_performance') }}
                    </h2>
                    <h3>{{ __('links.advanced_performance') }}</h3>
                    <p>
                        @if (LaravelLocalization::getCurrentLocale() === 'en')
                            Over 30 year, we have accumulated experience in matching the market need and securing the
                            supply. Professionalism in building materials company with innovative products and
                            differentiated solution.
                        @else
                            أكثر من 30 عامًا ، تراكمت لدينا خبرة في مطابقة احتياجات السوق وتأمين التوريد. الاحتراف في شركة
                            مواد البناء بمنتجات مبتكرة وحلول متباينة.
                        @endif
                    </p>
                </div>
            </div>
                    ...
                    <div class="col-md-6 col-lg-4 ftco-animate">
                        <div class="blog-entry">
                            <a href="#" class="block-20 d-flex align-items-end"
                                style="background-image: url('{{ asset('uploads/company_performances') }}/{{ $performance->image ?? '' }}');">
                            ...
                            <div class="text bg-white p-4">
                                <h3 class="heading"><a href="#">
                                        ...
                                        @else
                                            {{ $performance->ar_title }} @endif
                                    </a></h3>
                                <p>
                                    @if (LaravelLocalization::getCurrentLocale() === 'en')
                                        {{ $performance->en_subTitle }}
                                    @else
                                        {{ $performance->ar_subTitle }} @endif
                                </p>
        ...
        <div class="container">
            <div class="row justify-content-center mb-2 pb-2">
                <div class="col-md-8 text-center heading-section ftco-animate">
                    ...
                    <p>
                        @if (LaravelLocalization::getCurrentLocale() === 'en')
                            PREMIER UPVC always considers the importance of good external appearances as well as making a
                            healthy and comfortable living space. You can feel a more beautiful scene and fresh air through
                            the PREMIER
                        @else
                ...
                <div class="col-md-2 mb-3">
                    <ul class="nav nav-pills flex-column" id="myTab" role="tablist">
                        ...
                        <li class="nav-item" class="active">
                            <a class="nav-link" id="#home" data-toggle="tab" href="#home" aria-controls="home"
                                aria-selected="false"><i class="ion-ios-arrow-round-forward"></i>
                                @if (LaravelLocalization::getCurrentLocale() === 'en')
                                    All
                                @else
                                    الكل
                            ...
                            @foreach ($categories as $item)
                                <a class="nav-link" id="#home{{ $item->id }}" data-toggle="tab"
                                    href="#home{{ $item->id }}" aria-controls="home" aria-selected="false"><i
                                        class="ion-ios-arrow-round-forward"></i>
                                    @if (LaravelLocalization::getCurrentLocale() === 'en')
                                        {{ $item->en_name }}
                                    @else
                                        {{ $item->ar_name }}
                                ...
                                @foreach ($pro_cat as $cat)
                                    <div class="col-md-6 col-lg-4 ftco-animate">
                                        <div class="blog-entry">
                                            ...
                                            <div class="text bg-white p-4">
                                                <h3 class="heading text-center"><a
                                                        href="{{ LaravelLocalization::localizeUrl('/single-product/' . $cat->id) }}"
                                                        class="clr-green text-center">
                                                        @if (LaravelLocalization::getCurrentLocale() === 'en')
                                                            {{ $cat->en_name }}
                                                        @else
                                                            {{ $cat->ar_name }} @endif
