                    ...
                    <div class="row mt-5  {{ LaravelLocalization::getCurrentLocale() === 'ar' ? 'dir-rtl' : '' }}">
                        <div class="col-lg-6 ">
                            <div class="services-3 d-flex ">
                                <div class="icon mt-2 d-flex justify-content-center align-items-center "><span
                                        class="icon-file "></span></div>
                                <div class="text px-3 pt-2 pdf ">
                                    <p>
                                        @if (LaravelLocalization::getCurrentLocale() === 'en')
                                            <a href="{{ asset('uploads/companies') }}/{{ $company->company_catalogue_pdf }}"
                                                download>{{ __('links.catalogue_pdf') }}</a>
                                        @else
                                            <a href="{{ asset('uploads/companies') }}/{{ $company->ar_catalogue_pdf }}"
                                                download>{{ __('links.catalogue_pdf') }}</a>
                                        @endif
                                        @if (LaravelLocalization::getCurrentLocale() === 'en')
                                            <a href="{{ asset('uploads/companies') }}/{{ $company->company_catalogue_pdf }}"
                                                download><span class="icon-download pt-2 "></span></a>
                                        @else
                                            <a href="{{ asset('uploads/companies') }}/{{ $company->ar_catalogue_pdf }}"
                                                download><span class="icon-download pt-2 "></span></a>
                                        @endif
                        ...
                        <div class="col-lg-6 ">
                            <div class="services-3 d-flex ">
                                ...
                                <div class="text px-3 pt-2 pdf ">
                                    <p>
                                        <a href="{{ asset('uploads/companies') }}/{{ $company->company_profile_pdf }}"
                                            download>{{ __('links.dawnload_profile') }}</a>
        ...
        <div class="container ">
            <div class="row ">
                <div class="col-lg-12 ftco-animate ">
                    <h3 class="mb-4  {{ LaravelLocalization::getCurrentLocale() === 'ar' ? 'text-right' : '' }}">
                        {{ __('links.other_product') }}</h3>
                    <div class="row {{ LaravelLocalization::getCurrentLocale() === 'ar' ? 'dir-rtl' : '' }}">
                        @foreach ($others as $other)
                            <div class="col-md-6 col-lg-2 ftco-animate ">
                                <div class="blog-entry ">
                                    <a href="{{ LaravelLocalization::localizeUrl('/single-product/' . $other->id) }}"
                                        class="block-20 d-flex align-items-end related-img "
                                        style="background-image: url( '{{ asset('uploads/products') }}/{{ $other->master_image ?? '' }}'); "></a>
                                    <div class="text bg-white p-4 ">
                                        <h3 class="heading text-center "><a
                                                href="{{ LaravelLocalization::localizeUrl('/single-product/' . $other->id) }}"
