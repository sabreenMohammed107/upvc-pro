                    ...
                    <div class="sparkline13-hd">
                        <div class="main-sparkline13-hd" @if (app()->getLocale()=='ar') style="direction: rtl" @endif>
                            ...
                            <div>
                                ...
                                <a href="/AdminWhyCompany/create" class="btn btn-primary">Add New</a>
                                @else
                    ...
                    <div class="sparkline13-graph">
                        <div class="datatable-dashv1-list custom-datatable-overright">
                            ...
                            <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                ...
                                <tbody>
                                    @foreach ($why_company as $index=>$why_company)
                                        <tr>
                                            <td></td>
                                            <td>{{ $index + 1 }}</td>
                                            <td>{{ $why_company->en_title }}</td>
                                            <td>{{ $why_company->ar_title }}</td>
                                            <td>{{ $why_company->en_subTitle }}</td>
                                            <td>{{ $why_company->ar_subTitle }}</td>
                                            <td>
                                                @if (Auth::user()->hasPermission('users-update'))
                                                    <a href="/AdminWhyCompany/{{$why_company->id}}/edit" class="btn btn-warning mb-1"><i class="fa fa-edit"></i> Edit </a>
                                                @else
                                                    <a href="" class="btn btn-warning mb-1" disabled><i class="fa fa-edit"></i> Edit </a>
                                               @endif
                                                @if (Auth::user()->hasPermission('users-delete'))
                                                    <form action="{{ route('AdminWhyCompany.destroy',$why_company->id) }}" method="post" style="display: inline-block">
                                                        {{ csrf_field() }}
                                                        @method('DELETE')
